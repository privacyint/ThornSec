...
required. Anyone using this system consents to these terms and the laws
of the United Kingdom and United States respectively.
************************NOTICE***********************" | sudo tee /etc/ssh/banner > /dev/null
...
printf \"Currently logged in users:\t%s\n\" \${users}
echo
echo \"HERE BE DRAGONS.\"" | sudo tee /etc/update-motd.d/00-motd > /dev/null
...
#============ no_unexpected_users =============
out=$(awk -F':' '{ print $1 }' /etc/passwd | egrep -v "^ed\$" | egrep -v "^root\$" | egrep -v "^daemon\$" | egrep -v "^bin\$" | egrep -v "^sys\$" | egrep -v "^sync\$" | egrep -v "^games\$" | egrep -v "^man\$" | egrep -v "^lp\$" | egrep -v "^mail\$" | egrep -v "^news\$" | egrep -v "^uucp\$" | egrep -v "^proxy\$" | egrep -v "^www-data\$" | egrep -v "^backup\$" | egrep -v "^list\$" | egrep -v "^irc\$" | egrep -v "^gnats\$" | egrep -v "^nobody\$" | egrep -v "^systemd-timesync\$" | egrep -v "^systemd-network\$" | egrep -v "^systemd-resolve\$" | egrep -v "^systemd-bus-proxy\$" | egrep -v "^_apt\$" | egrep -v "^messagebus\$" | egrep -v "^sshd\$" | egrep -v "^statd\$" | egrep -v "^unbound\$" | tee /dev/stderr | grep -v 'tee /dev/stderr');
test="";
if [ "${out}" != "${test}" ] ; then
