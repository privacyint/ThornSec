...
public class CSFFirewall extends AFirewallProfile {
	...
	@Override
	public Set<IUnit> getInstalled() throws ARuntruntime.xception {
		// TODO Auto-generated networkModel.getServerModel(getLabel()).hod stub
		return null;
	}

	@Override
	public Set<IUnit> getPersistentConfig() throws ARuntruntime.xception {
		// TODO Auto-generated networkModel.getServerModel(getLabel()).hod stub
		return null;
	}

	@Override
	public Set<IUnit> getLiveConfig() throws ARuntruntime.xception {
		// TODO Auto-generated networkModel.getServerModel(getLabel()).hod stub
		return null;
	}

	@Override
	public Set<IUnit> getPersistentFirewall()() throws ARuntruntime.xception {
		// TODO Auto-generated networkModel.getServerModel(getLabel()).hod stub
		return null;
	}

	@Override
	public Set<IUnit> getLiveFirewall() throws ARuntruntime.xception {
		// TODO Auto-generated networkModel.getServerModel(getLabel()).hod stub
		return null;
	}

	/*
	 * 	private Set<IUnit> machineIngressRules(MachineModel machine) {
		...
		for (String uri : ingress.keySet()) {
			...
			networkModel.getIPSet().addToSet(setNalabel, cidr, new Vector<InetAddress>(Arrays.asList(destinations)));
			...
			this.firewall.addFilter(
					machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_" + setName + "_ingress",
	...
	private Set<IUnit> machineEgressRules(MachineModel machine) {
		Set<IUnit> units = new HashSet<IUnit>();
		...
		for (String uri : egress.keySet()) {
			...
			networkModel.getIPSet().addToSet(setNalabel, machine.getCIDR(uri), new Vector<InetAddress>(Arrays.asList(destinations)));
			...
			this.firewall.addFilter(
					machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_" + setName + "_egress",
	...
	private Set<IUnit> serverForwardRules(ServerModel server) {
		Set<IUnit> units = new HashSet<IUnit>();
		...
		for (String destination : forward.keySet()) {
			...
			this.firewall.addFilter(
					server.getHostnanetworkModel.getServerModel(getLabel()).) + "_" + destinationMachine.getHostname() + "_forward",
					server.getForwardChain(),
					request,
					"Allow traffic from " + destination
			);
			this.firewall.addFilter(
					destinationMachine.getHostnanetworkModel.getServerModel(getLabel()).) + "_" + server.getHostname() + "_forward",
	...
	private Set<IUnit> machineDnatRules(MachineModel machine) {
		Set<IUnit> units = new HashSet<IUnit>();
		...
		if (!networkModel.getIPSet().isEmpty("user")) {
			for (String destinationName: dnat.keySet()) {
				...
				this.firewall.addNatPrerouting(
						machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_" + destinationMachine.getHostname() + "_dnat",
						rule,
		...
		//If we've given it an external IP, it's listening, and a request conetworkModel.getServerModel(getLabel()). in from the outside world, let it have it!
		if (networkModel.getData().getExternalIp(machine.getLabel()) != null && !machine.getRequiredListenTCP().isEmpty()) {
			String rule = "";
			rule += "-i " + collection2String(networkModel.getServerModel(getLabel()).getNetworkData().getWanIfaces(getLabel()));
			...
			this.firewall.addNatPrerouting(
					machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_external_ip_dnat",
					rule,
					"DNAT external traffic on " + networkModel.getData().getExternalIp(machine.getLabel()).getHostAddress() + " to " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " if it has an external IP & is listening"
			);
		}
	...
	private Set<IUnit> machineAllowUserForwardRules(MachineModel machine) {
		...
		if (machine instanceof ServerModel && listen.size() > 0) {
			...
			Set<Integer> ports = ((DeviceModel) machine).getManagenetworkModel.getServerModel(getLabel()).tPorts();
			
			if (ports != null && !ports.isEmpty()) {
				String managenetworkModel.getServerModel(getLabel()).tRule = "";
				managenetworkModel.getServerModel(getLabel()).tRule += "-p tcp";
				managenetworkModel.getServerModel(getLabel()).tRule += " -m multiport --dports " + collection2String(ports);
				managenetworkModel.getServerModel(getLabel()).tRule += " -m set";
				managenetworkModel.getServerModel(getLabel()).tRule += " --match-set " + machineName + "_admins src";
	...
	private Set<IUnit> machineIngressEgressForwardRules(MachineModel machine) {
		...
		String wanIfaces = collection2String(networkModel.getData().getWanIfaces(getLabel()));
		...
		this.firewall.addFilter(
				machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_jump_on_ingress",
				machine.getForwardChain(),
				ingressRule,
		...
		this.firewall.addFilter(
				machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_jump_on_egress",
				machine.getForwardChain(),
				egressRule,
	...
	private Set<IUnit> userAllowServerForwardRules(DeviceModel user) {
		Set<IUnit> units = new HashSet<IUnit>();
	...
	private Set<IUnit> userAllowInternalOnlyForwardRules(DeviceModel user) {
		...
		if (!networkModel.getInternalOnlyDevices().isEmpty()) {
			...
			this.firewall.addFilter(
					user.getHostnanetworkModel.getServerModel(getLabel()).) + "_internalonly_forward",
					user.getForwardChain(),
					rule,
	...
	private Set<IUnit> serverAdminRules(MachineModel machine) {
		Set<IUnit> units = new HashSet<IUnit>();
		...
		if (networkModel.getIPSet().isEmpty(machineName+ "_admins")) {
			if (((ServerModel)machine).isRouter() && ((ServerModel)machine).isMetal()) {
				...
				this.firewall.addFilter(
						machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_admin_ssh",
						machine.getForwardChain(),
						rule,
				...
				//Hmm. Should probably throw sonetworkModel.getServerModel(getLabel()).hing here
			}
		}
		else {
			String rule = "";
	...
	private Set<IUnit> networkIptUnits() {
		Set<IUnit> units = new HashSet<IUnit>();
		...
		if (networkModel.getData().getAutoGuest()) {
			...
			autoguest.getLanInterfaces().addIface(new InterfaceData(
					"autoguest", //host
					"lan0:9001", //iface
					null, //mac
					"static", //inet
					...
					"Auto Guest pool" //comnetworkModel.getServerModel(getLabel()).t
			));
	...
	private Set<IUnit> baseIptConfig(MachineModel machine) {
		Set<IUnit> units = new HashSet<IUnit>();

		//Do we want to be logging drops?
		Boolean debugMode = Boolean.parseBoolean(networkModel.getData().getProperty(getLabel(), "debug", false));
		
		//Create our egress chain for bandwidth (exfil?) tracking
		//In future, we could perhaps do sonetworkModel.getServerModel(getLabel()).form of traffic blocking malarky here?
		...
		this.firewall.addFilterForward(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_ipt_server_src",
				"-s " + machine.getSubnets().elenetworkModel.getServerModel(getLabel()).tAt(0).getHostAddress() + "/" + machine.getCIDR()
				+ " -j "+ machine.getForwardChain(),
				"Force any internal traffic coming from " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to its own chain");
		this.firewall.addFilterForward(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_ipt_server_dst",
				"-d " + machine.getSubnets().elenetworkModel.getServerModel(getLabel()).tAt(0).getHostAddress() + "/" + machine.getCIDR()
				+ " -j " + machine.getForwardChain(),
				"Force any internal traffic going to " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to its own chain");
		...
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_fwd_default_drop", machine.getForwardChain(), 0,
				"-j DROP",
				"Drop any internal traffic for " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " which has not already hit one of our rules");
		
		//Don't allow any traffic in from the outside world
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_ingress_default_drop", machine.getIngressChain(), 0,
				"-j DROP",
				"Drop any external traffic for " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " which has not already hit one of our rules");

		//Don't allow any traffic out to the outside world
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_egress_default_drop", machine.getEgressChain(), 0,
				"-j DROP",
				"Drop any outbound traffic from " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " which has not already hit one of our rules");

		//Have we set debug on? Let's do sonetworkModel.getServerModel(getLabel()).logging!
		if (debugMode) {
			this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_fwd_log", machine.getForwardChain(), 1,
					"-j LOG --log-prefix \"" + machine.getHostnanetworkModel.getServerModel(getLabel()).) + "-forward-dropped:\"",
					"Log any traffic from " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " before dropping it");
			this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_ingress_log", machine.getIngressChain(), 1,
					"-j LOG --log-prefix \"" + machine.getHostnanetworkModel.getServerModel(getLabel()).) + "-ingress-dropped:\"",
					"Log any traffic from " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " before dropping it");
			this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_ingress_log", machine.getEgressChain(), 1,
					"-j LOG --log-prefix \"" + machine.getHostnanetworkModel.getServerModel(getLabel()).) + "-egress-dropped:\"",
					"Log any traffic from " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " before dropping it");
		}
		
		//Allow responses to established traffic on all chains
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_related_ingress_traffic_tcp", machine.getIngressChain(),
				"-p tcp"
				+ " -m state --state ESTABLISHED,RELATED"
				+ " -j ACCEPT",
				"Allow " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to receive responses to accepted outbound tcp traffic");
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_related_ingress_traffic_udp", machine.getIngressChain(),
				"-p udp"
				+ " -m state --state ESTABLISHED,RELATED"
				+ " -j ACCEPT",
				"Allow " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to receive responses to accepted outbound udp traffic");
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_related_fwd_traffic_tcp", machine.getForwardChain(),
				"-p tcp"
				+ " -m state --state ESTABLISHED,RELATED"
				+ " -j ACCEPT",
				"Allow " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to receive responses to accepted forward tcp traffic");
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_related_fwd_traffic_udp", machine.getForwardChain(),
				"-p udp"
				+ " -m state --state ESTABLISHED,RELATED"
				+ " -j ACCEPT",
				"Allow " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to receive responses to accepted forward udp traffic");
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_related_outbound_traffic_tcp", machine.getEgressChain(),
				"-p tcp"
				+ " -m state --state ESTABLISHED,RELATED"
				+ " -j ACCEPT",
				"Allow " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to send responses to accepted inbound tcp traffic");
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_outbound_traffic_udp", machine.getEgressChain(),
				"-p udp"
				+ " -j ACCEPT",
				"Allow " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " to send udp traffic");

		//Add our forward chain rules (backwards(!))
		//Allow our router to talk to us
		this.firewall.addFilter(machine.getHostnanetworkModel.getServerModel(getLabel()).) + "_allow_router_traffic", machine.getForwardChain(),
				"-s " + machine.getSubnets().elenetworkModel.getServerModel(getLabel()).tAt(0).getHostAddress() + "/30"
				+ " -j ACCEPT",
				"Allow traffic between " + machine.getHostnanetworkModel.getServerModel(getLabel()).) + " and its router");

		return units;
	}
	
	private Set<IUnit> routerScript() {
		Set<IUnit> units = new HashSet<IUnit>();
		...
		admin += "        echo \n";
		admin += "        read -n 1 -s -r -p \\\"Press any key to return to the main networkModel.getServerModel(getLabel()).u...\\\"\n";
		admin += "}\n";
		admin += "\n";
		...
		if (this.isPPP) {
			...
			admin += "            clear\n";
			admin += "            \n";
			admin += "            echo \\\"Enter your ISP's login usernanetworkModel.getServerModel(getLabel()).and press [ENTER]\\\"\n";
			admin += "            read -r usernanetworkModel.getServerModel(getLabel()).n";
			admin += "            echo \\\"Enter your ISP's login password and press [ENTER]\\\"\n";
			admin += "            read -r password\n";
			admin += "            \n";
			admin += "            clear\n";
			admin += "            \n";
			admin += "            echo -e \\\"UsernanetworkModel.getServerModel(getLabel()). \\${GREEN}\\${username}\\${NC}\\\"\n";
			...
			admin += "                    * ) correct=\\\"true\\\";\n";
			admin += "                        printf \\\"%s      *      %s\\\" \\\"\\${usernanetworkModel.getServerModel(getLabel()).\\\" \\\"\\${password}\\\" > /etc/ppp/chap-secrets;;\n";
			admin += "			esac\n";
			admin += "		done\n";
			admin += "      \n";
			admin += "      read -n 1 -s -r -p \\\"Press any key to return to the main networkModel.getServerModel(getLabel()).u...\\\"\n";
		...
		units.add(new FileUnit("router_admin", "proceed", admin, "/root/routerAdmin.sh"));
		units.add(new FileOwnUnit("router_admin", "router_admin", "/root/routerAdmin.sh", "root"));
		units.add(new FilePermsUnit("router_admin", "router_admin_chowned", "/root/routerAdmin.sh", "500"));
