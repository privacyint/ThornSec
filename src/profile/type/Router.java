...
public class Router extends AStructuredProfile {
	 ...
	 */
	public final void addMACVLAN(MachineType type, String vlanNetwork) throws InvalidIPAddressException, InvalidMachineModelException {
		IPAddress ip = null;
		...
		try {
			final Collection<NetworkInterfaceData> nicsData = getNetworkModel().getData().getNetworkInterfaces(getLabel()).get(Direction.LAN);
		...
		try {
			final Collection<NetworkInterfaceData> nicsData = getNetworkModel().getData().getNetworkInterfaces(getLabel()).get(Direction.WAN);
			for (final NetworkInterfaceData nicData : nicsData) {
				final String nicName = nicData.getIface();
