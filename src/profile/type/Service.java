...
public class Service extends AStructuredProfile {
	...
	throws InvalidServerModelException {
		...
		units.add(new SimpleUnit("is_virtualbox_guest", "proceed", "",
		...
		networkModel.getServerModel(getLabel()).getUserModel().addUsername("vboxadd");
		networkModel.getServerModel(getLabel()).addProcessString("\\[iprt-VBoxWQueue\\]$");
		networkModel.getServerModel(getLabel()).addProcessString("/usr/sbin/VBoxService --pidfile /var/run/vboxadd-service.sh$");
		...
		networkModel.getServerModel(getLabel()).addProcessString("/usr/sbin/haveged --Foreground --verbose=1 -w 1024$");
