...
public class SSH extends AStructuredProfile {
	...
	throws InvalidServerModelException, InvalidUserException, InvalidMachineException {
		...
		sshdConf.appendLine("Port " + networkModel.getData().getSSHPort(getLabel()));
		//sshdConf.appendLine((((ServerModel)me).isRouter()) ? "ListenAddress " + networkModel.getData().getIP().getHostAddress() + "\n" : "";
		...
		sshdConf.appendLine("MACs hmac-sha2-512-etm@openssh.com,hmac-sha2-256-etm@openssh.com,hmac-ripemd160-etm@openssh.com,umac-128-etm@openssh.com,hmac-sha2-512,hmac-sha2-256,hmac-ripemd160,umac-128@openssh.com");
		sshdConf.appendLine("Ciphers chacha20-poly1305@openssh.com,aes256-gcm@openssh.com,aes128-gcm@openssh.com,aes256-ctr,aes192-ctr,aes128-ctr");
		...
		networkModel.getServerModel(getLabel()).getConfigsModel().addConfigFilePath("/etc/ssh/sshd_config");

		//This banner is taken from https://www.dedicatedukhosting.com/hosting/adding-ssh-welcome-and-warning-messages/
		FileUnit banner = new FileUnit("sshd_banner", "proceed", "/etc/ssh/banner");
		units.add(banner);
		...
		networkModel.getServerModel(getLabel()).getConfigsModel().addConfigFilePath("/etc/ssh/banner");
		...
		// Released under GPL
		FileUnit motd = new FileUnit("sshd_motd", "proceed", "/etc/update-motd.d/00-motd");
		units.add(motd);
		...
		networkModel.getServerModel(getLabel()).getConfigsModel().addConfigFilePath("/etc/update-motd.d/00-motd");
	...
	throws InvalidServerModelException, InvalidPortException {
		...
		networkModel.getServerModel(getLabel()).addListen(Encapsulation.TCP, networkModel.getData().getSSHPort(getLabel()));
	...
	throws InvalidUserException, InvalidServerException, InvalidMachineException {
		...
		for (String admin : networkModel.getData().getAdmins(getLabel())) {
			...
			units.add(new FileUnit("ssh_key_" + admin, "ssh_dir_" + admin + "_created", networkModel.getData().getUserSSHKey(admin), keys));
