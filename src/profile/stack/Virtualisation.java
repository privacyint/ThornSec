...
public class Virtualisation extends AStructuredProfile {
	...
	protected Set<IUnit> getInstalled() {
		...
		units.add(new InstalledUnit("metal_virtualbox", "virtualbox_pgp", "virtualbox-6.0"));
	...
	@Override
	protected final Set<IUnit> getPersistentConfig()
	throws InvalidServerModelException {
		Set<IUnit> units = new HashSet<IUnit>();

		networkModel.getServerModel(getLabel()).addProcessString("/usr/lib/virtualbox/VBoxXPCOMIPCD$");
		networkModel.getServerModel(getLabel()).addProcessString("/usr/lib/virtualbox/VBoxSVC --auto-shutdown$");
		networkModel.getServerModel(getLabel()).addProcessString("\\[iprt-VBoxWQueue\\]$");
		networkModel.getServerModel(getLabel()).addProcessString("\\[iprt-VBoxTscThr\\]$");
		networkModel.getServerModel(getLabel()).addProcessString("\\[kvm-irqfd-clean\\]$");
	...
	throws InvalidServerModelException {
		...
		networkModel.getServerModel(getLabel()).getAptSourcesModel().addAptSource("virtualbox", "deb http://download.virtualbox.org/virtualbox/debian stretch contrib", "keyserver.ubuntu.com", "0xa2f683c52980aecf");
		networkModel.getServerModel(getLabel()).addEgress("download.virtualbox.org");
	...
	String preseed(String service, Boolean expirePasswords) {
		...
		HostName debianMirror    = networkModel.getData().getDebianMirror(service);
	...
	Set<IUnit> buildIso(String service, String preseed) {

		Set<IUnit> units = new HashSet<IUnit>();
		
		String isoDir =  networkModel.getData().getHypervisorThornsecBase(getLabel()) + "/isos/" + service + "/";
		...
		units.add(new FileUnit("preseed_" + service, "debian_netinst_iso_" + cleanedFilename + "_downloaded", preseed, isoDir + "preseed.cfg"));
		...
		buildIso += 	" mount -o loop " + networkModel.getData().getHypervisorThornsecBase(getLabel()) + "/" + filename + " loopdir;";
		buildIso += 	" mkdir cd;";
	...
	Set<IUnit> buildServiceVm(String service, String bridge) {
		String baseDir = networkModel.getData().getHypervisorThornsecBase(getLabel());
		...
		units.add(new DirOwnUnit("boot_disk_dir_" + service, "boot_disk_dir_" + service + "_created", bootDiskDir, user, group));
		units.add(new DirPermsUnit("boot_disk_dir_" + service, "boot_disk_dir_" + service + "_chowned", bootDiskDir, "750"));
	
		units.add(new DirUnit("data_disk_dir_" + service, "proceed", dataDiskDir));
		units.add(new DirOwnUnit("data_disk_dir_" + service , "data_disk_dir_" + service + "_created", dataDiskDir, user, group));
		units.add(new DirPermsUnit("data_disk_dir_" + service, "data_disk_dir_" + service + "_chowned", dataDiskDir, "750"));
		...
		units.add(new FileUnit(service + "_mark_backup_dir", "backup_dir_" + service + "_chmoded" , "In memoriam Luke and Guy.  Miss you two!", backupTargetDir + "/backup.marker"));
		...
		//units.add(new DirOwnUnit("boot_disk_loopback_dir_" + service, "boot_disk_loopback_dir_" + service + "_created", bootLoopbackDir, "root", "root"));
		//units.add(new DirPermsUnit("boot_disk_loopback_dir_" + service, "boot_disk_loopback_dir_" + service + "_chowned", bootLoopbackDir, "755"));
		//And, more importantly, the data disk
		units.add(new DirUnit("data_disk_loopback_dir_" + service, "proceed", dataLoopbackDir + "/"));
		//units.add(new DirOwnUnit("data_disk_loopback_dir_" + service, "data_disk_loopback_dir_" + service + "_created", dataLoopbackDir, "root", "root"));
		//units.add(new DirPermsUnit("data_disk_loopback_dir_" + service, "data_disk_loopback_dir_" + service + "_chowned", dataLoopbackDir, "755"));
		...
		units.add(new FileOwnUnit(service + "_boot_disk", service + "_boot_disk", bootDiskImg + diskExtension, user, group));
		...
		units.add(new FileOwnUnit(service + "_data_disk", service + "_data_disk", dataDiskImg + diskExtension, user, group));
		...
		units.add(new SimpleUnit(service + "_guest_additions_iso_attached", service + "_sas_controller",
		...
		units.add(modifyVm(service, user, "ioapic", "on", "IO APIC couldn't be enabled for " + service + ".  This is required for 64-bit installations, and for more than 1 virtual CPU in a service."));
		units.add(modifyVm(service, user, "hwvirtex", "on"));
		units.add(modifyVm(service, user, "pae", "on"));
		units.add(modifyVm(service, user, "cpus", networkModel.getData().getCpus(service)));

		//RAM setup
		units.add(modifyVm(service, user, "memory", networkModel.getData().getRam(service)));
		...
		units.add(modifyVm(service, user, "boot1", "disk", "Couldn't set the boot order for " + service + ".  This may mean the service will not be installed.", service + "_sas_controller"));
		units.add(modifyVm(service, user, "boot2", "dvd", "Couldn't set the boot order for " + service + ".  This may mean the service will not be installed.", service + "_sas_controller"));
		...
		for (InterfaceData lanIface : networkModel.getMachineModel(service).getInterfaces()) { //networkModel.getData().getLanIfaces(service).entrySet() ) {
			units.add(modifyVm(service, user, "nic" + i, "bridged", "Couldn't give " + service + " a connection to the network.  This means the service will not be able to talk to the router or network, and will not be installed."));
			units.add(modifyVm(service, user, "bridgeadapter" + i, bridge, "Couldn't give " + service + " a connection to the network.  This means the service will not be able to talk to the router or network, and will not be installed.", service + "_nic1_bridged"));
			units.add(modifyVm(service, user, "nictype" + i, "82545EM", "Couldn't set " + service + "'s network adapter to use the 82545EM model.", service + "_bridgeadapter1_" + bridge));
			units.add(modifyVm(service, user, "macaddress" + i, lanIface.getMac().replace(":", "").toUpperCase(), "Couldn't set " + service + "'s MAC address.  This means the service will not be able to get an IP address, and will not be installed."));
			++i;
		}
		...
		units.add(guestPropertySet(service, user, "timesync-interval", "10000", "Couldn't sync the clock between " + service + " and its metal.  You'll probably see some clock drift in " + service + " as a result."));
		units.add(guestPropertySet(service, user, "timesync-min-adjust", "100", "Couldn't sync the clock between " + service + " and its metal.  You'll probably see some clock drift in " + service + " as a result."));
		units.add(guestPropertySet(service, user, "timesync-set-on-restore", "1", "Couldn't sync the clock between " + service + " and its metal.  You'll probably see some clock drift in " + service + " as a result."));
		units.add(guestPropertySet(service, user, "timesync-set-threshold", "1000", "Couldn't sync the clock between " + service + " and its metal.  You'll probably see some clock drift in " + service + " as a result."));
		...
		networkModel.getServerModel(getLabel()).addProcessString("/usr/lib/virtualbox/VBoxHeadless --comment " + service + " --startvm `if id '" + user + "' >/dev/null 2>&1; then sudo -u " + user + " bash -c 'VBoxManage list runningvms | grep " + service + "' | awk '{ print $2 }' | tr -d '{}'; else echo ''; fi` --vrde config *$");
		networkModel.getServerModel(getLabel()).addProcessString("awk \\{");
		networkModel.getServerModel(getLabel()).addProcessString("tr -d \\{\\}$");
