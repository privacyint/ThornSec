...
public class OnionBalance extends AStructuredProfile {
	...
	protected Set<IUnit> getInstalled() throws InvalidServerModelException {
		...
		networkModel.getServerModel(getLabel()).getUserModel().addUsername("debian-tor");
		networkModel.getServerModel(getLabel()).getUserModel().addUsername("onionbalance");
	...
	throws InvalidServerModelException {
		...
		units.addAll(networkModel.getServerModel(getLabel()).getBindFsModel().addDataBindPoint("onionbalance", "onionbalance_installed", "onionbalance", "onionbalance", "0700"));
		units.addAll(networkModel.getServerModel(getLabel()).getBindFsModel().addLogBindPoint("onionbalance", "onionbalance_installed", "onionbalance", "0750"));
		...
		FileUnit service = new FileUnit("onionbalance_service", "onionbalance_installed", "/lib/systemd/system/onionbalance.service");
		units.add(service);
		...
		service.appendLine("ExecStop=-/sbin/start-stop-daemon --quiet --stop --retry=TERM/5/KILL/5 --pidfile /run/onionbalance.pid");
		...
		FileUnit torrc = new FileUnit("torrc", "tor_installed", "/etc/tor/torrc");
		units.add(torrc);
		...
		units.add(new SimpleUnit("tor_service_enabled", "torrc",
	...
	throws InvalidServerModelException, InvalidPropertyArrayException, InvalidMachineException {
		...
		FileUnit onionbalanceConfig = new FileUnit("onionbalance", "onionbalance_installed", "/etc/onionbalance/config.yaml");
		...
		Set<String> backends = networkModel.getData().getPropertyArray(getLabel(), "backend");
	...
	throws InvalidServerModelException {
		...
		networkModel.getServerModel(getLabel()).getAptSourcesModel().addAptSource("tor", "deb http://deb.torproject.org/torproject.org stretch main", "keys.gnupg.net", "A3C4F0F979CAA22CDBA8F512EE8CBC9E886DDD89");
		networkModel.getServerModel(getLabel()).addEgress("*"); //Needs to be able to call out to everywhere
