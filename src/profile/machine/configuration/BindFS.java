...
public class BindFS extends AModel {
	...
	public Collection<IUnit> addLogBindPoint(String name, String precondition, String username, String permissions) {
		return addBindPoint(name + "_log", precondition, "/var/log/." + name.replaceAll("-", "_"),
				"/var/log/" + name.replaceAll("-", "_"), username, username, permissions, "/var/log", true);
	}

	public Collection<IUnit> addDataBindPoint(String name, String precondition, String username, String group,
			String permissions) {
		return addBindPoint(name + "_data", precondition, "/media/metaldata/" + name.replaceAll("-", "_"),
				"/media/data/" + name.replaceAll("-", "_"), username, group, permissions, "/media/metaldata", false);
	}
	...
	public Collection<IUnit> addBindPoint(String name, String precondition, String baseDirectory, String bindPoint,
		...
		// Add to our fstab
		units.add(new FileAppendUnit("fstab", name + "_bindpoint_created",
				baseDirectory + " " + bindPoint + " fuse.bindfs force-user=" + username + ",force-group=" + group
						+ ",create-for-user=" + username + ",create-for-group=" + group + ",perms=" + permissions
						+ requires + " 0 0",
				"/etc/fstab", "Couldn't add " + bindPoint
						+ " to our fstab.  This means the directory will have the wrong permissions and there will be other failures."));

		// Mount!
		units.add(new DirMountedUnit(name, "fstab_appended", bindPoint, "Couldn't mount " + bindPoint
				+ ".  This means the directory will have the wrong permissions and there will be other failures."));

		// @TODO: FIXME
		// networkModel.getData().getServerData(getLabel()).getProcessModel().addProcess("bindfs
		// " + baseDirectory + " " + bindPoint + " -o rw,force-user=" + username +
		// ",force-group=" + group + ",create-for-user=" + username +
		// ",create-for-group=" + group + ",perms=" + permissions + ",dev,suid$");
