...
public class SVN extends AStructuredProfile {
	...
	public Collection<IUnit> getPersistentConfig() throws InvalidServerException, InvalidMachineModelException {
		...
		units.add(new SimpleUnit("apache_mod_dav_enabled", "libapache2_svn_installed", "sudo a2enmod dav;",
				"sudo apache2ctl -M | grep dav", "", "fail"));

		units.add(new SimpleUnit("apache_mod_dav_fs_enabled", "libapache2_svn_installed", "sudo a2enmod dav_fs;",
				"sudo apache2ctl -M | grep dav_fs", "", "fail"));

		units.add(new SimpleUnit("apache_mod_auth_digest_enabled", "apache_installed", "sudo a2enmod auth_digest;",
				"sudo apache2ctl -M | grep auth_digest", "", "fail"));

		units.add(new SimpleUnit("apache_mod_dav_svn_enabled", "libapache2_svn_installed", "sudo a2enmod dav_svn;",
				"sudo apache2ctl -M | grep dav_svn", "", "fail"));

		// Turn off Apache version on error pages
		units.add(new FileAppendUnit("hide_apache_version_errors", "apache_installed", "ServerSignature Off",
		...
		units.add(new FileDownloadUnit("svn_admin", "apache_installed",
				"https://kent.dl.sourceforge.net/project/ifsvnadmin/svnadmin-1.6.2.zip", "/root/svnadmin.zip"));
		...
		units.add(new FileEditUnit("svn_admin_php_version", "svn_admin_unzipped",
				...
				"if (false) { /* removed php version check */", "/media/data/www/admin/include/config.inc.php",
	...
	public Collection<IUnit> getLiveConfig() {
		...
		units.add(new FileUnit("default_apache_conf", "apache_installed", apacheConf,
		...
		units.add(new FileUnit("apache_svn_conf", "apache_mod_dav_svn_enabled", davConf,
				"/etc/apache2/mods-available/dav_svn.conf"));

		units.add(new SimpleUnit("svn_admin_password", "apache_installed",
				"SVN_ADMIN_PASSWORD=`openssl rand -hex 32`;"
						+ "SVN_DIGEST=`echo -n \"admin:thornsecsvn:\" && echo -n \"admin:thornsecsvn:${SVN_ADMIN_PASSWORD}\" | md5sum | awk '{print $1}'`;" // Can't
																																							// pass
																																							// to
																																							// htdigest
																																							// so
		...
		units.add(new FileUnit("svn_admin_config_file", "svn_admin_dir_created", svnConf,
				"/media/data/www/admin/data/config.ini"));
